...
- 观测: 与 rl_env 相同的感知管道 (mss抓屏 → YOLO + 血量 + m0b_state → 159维)
  注意: 此处存的是"感知层"159 维; rl_env 部署时 obs=160 维(末维 fire_since
  射击时序上下文由 env 实时拼)。BC 训练(bc_train.py)会从本文件的动作序列
  离线补算 fire_since → 160 维, 两端语义一致, 无需在本脚本里加。
