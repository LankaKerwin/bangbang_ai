...
> ⚠️ **声明：本项目仍在开发中，尚未完善**（当前进度：YOLO v1 感知可用、M1 雏形自动循环跑通、BC+PPO 链路就绪；
> 2026-09-07 因开学暂停归档，输入层改合成鼠标方案待实施）。代码与配置随时会变动，**请勿随意下载使用**，仅供参考学习。

用 **YOLO 感知 + OCR + RL(PPO)** 训练 AI 玩《枪豆人》。当前处于 **M0a/M0b 感知完成 → 输入设备切换(合成鼠标) → BC/PPO 长训** 阶段。

> 设计文档：
> - `docs/plan_overview.md` — 完整方案综述（感知/状态/动作/RL/课程/里程碑）
> - `docs/enemy_behavior_table.md` — 敌人/Boss 类型-行为知识库（RL 先验来源）
> - `docs/NEXT_STEP_mouse_input.md` — ⭐ 恢复工作的交接方案（vg→合成鼠标输入层 + 验证流程）
> - `docs/PORTFOLIO.md` — 作品集：里程碑/踩坑/决策理由/奖励哲学（含自驱动陷阱与过夜教训）
...
- [x] 录制素材并标注 → 训练 YOLO v1（mAP50 0.995）
- [x] M0b 弹幕栅格 + OCR + 状态融合（159 维）
- [x] M1 脚本教练 → M2 BC(人类示范预热) → M3 PPO 链路就绪
- [x] 击杀/Boss 奖励、死亡自重启、中途存档、回合统计
- [x] BC 自驱动陷阱定位（弃 fire_since 特征，走 159 维）
- [ ] 输入层 vg→合成鼠标/空格（见 `docs/NEXT_STEP_mouse_input.md`）→ 短稳 4096 → 长训观察
